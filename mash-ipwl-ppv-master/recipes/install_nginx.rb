...
file [nginx_dir+'/scripts/client_ip.sh'] do
  content '/usr/local/bin/aws s3 sync s3://ndm-ppv-ipwl/con/allowed_ips/ /usr/ip --exclude "global*" --exclude "*" --include "*com*" --sse=aws:kms --sse-kms-key-id arn:aws:kms:us-east-1:949421251013:key/19467aff-bc6f-447e-8adc-faea9013ceea'
  owner 'root'
  group 'root'
  mode '0755'
  action :create
...
file [nginx_dir+'/scripts/maintenance.sh'] do
  content '/usr/local/bin/aws s3 sync s3://ndm-ppv-maintenance/ /etc/nginx/maintenance  --delete  --sse=aws:kms --sse-kms-key-id arn:aws:kms:us-east-1:949421251013:key/19467aff-bc6f-447e-8adc-faea9013ceea'
  owner 'root'
  group 'root'
  mode '0755'
  action :create
