...
node.default['mash-ipwl-testing']['default_nameserver'] = get_default_nameserver()

# Sites Config
if node['mash-ipwl-testing']['sites_config'] == 'attribute'
  sites = node['mash-ipwl-testing']['sites']
else
  s3 = Aws::S3::Client.new(:region => node['mash-ipwl-testing']['s3_region'])

  sites = {}
  
  s3_sites_config = node['mash-ipwl-testing']['s3_sites_config']
  ...
  node.default['mash-ipwl-testing']['sites'] = sites
end
...
sites.each do |name, site_props|
  ...
  site_props['locations'].each do |location|
    secrets[site_props['domain']][location['name']] = Digest::SHA256.hexdigest(node['mash-ipwl-testing']['secret_salt']+site_props['domain']+location['name']+Time.new.strftime("%Y-%m-%d"))
    app_domains[site_props['domain']][location['name']] = {}
  end
end

# IP Whitelist
if node['mash-ipwl-testing']['ips_config'] == 'attribute'
  ip_whitelist = node['mash-ipwl-testing']['ip_whitelist'].to_hash
  trusted_proxies = node['mash-ipwl-testing']['trusted_proxies'].to_hash
  ...
  ip_whitelist_config = node['mash-ipwl-testing']['ip_whitelist_config']
    ...
    csv.each do |line|
      ...
      if (range_only <= 24)
        if domain_name == "global"
          ...
          if (node['mash-ipwl-testing']['mode_tableau'])
            range_whitelist[domain_name] ||= {}
  ...
  trusted_proxies_config = node['mash-ipwl-testing']['trusted_proxies_config']
...
template nginx_dir_temp+'/sites-enabled/default' do
    source 'default.erb'
    variables({
      :default_url => node['mash-ipwl-testing']['default_url'],
      :ipwl_atts => node['mash-ipwl-testing']
    })
end
...
sites.each do |name, site|
  template nginx_dir_temp+"/sites-enabled/#{site['domain']}" do
    ...
    variables({
        :site => site,
        :default_url => node['mash-ipwl-testing']['default_url'],
        :default_nameserver => node['mash-ipwl-testing']['default_nameserver'],
        :ipwl_atts => node['mash-ipwl-testing'],
    })
  end
