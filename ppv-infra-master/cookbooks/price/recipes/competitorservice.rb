...
config = data_bag_item(databag_name, node['price']['competitorservice']['databag_item'])
...
competitorservice_version = versions['competitorservice_version']
...
template "/opt/periscope/#{component_name}/#{component_name}.conf" do
  source 'competitorservice_config.erb'
  mode '0644'
