...
config = data_bag_item(databag_name, node['price']['configurationservice']['databag_item'])
...
configurationservice_version = versions['configurationservice_version']
...
component_name = 'price-configurationservice'
log_directory = "/opt/periscope/#{component_name}/logs"

yum_package component_name do
  version configurationservice_version
  flush_cache [:before]
...
template "/opt/periscope/#{component_name}/#{component_name}.conf" do
  source 'configurationservice_config.erb'
  mode '0644'
