...
resource "aws_iam_role_policy_attachment" "ppv_policy_get_bitdefender" {
  ...
  role       = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.bit[count.index]}"
...
resource "aws_iam_role_policy_attachment" "stg-ec2_autoscaling_access" {
  ...
  role       = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.stg_ec2[count.index]}"
