...
resource "aws_iam_role" "pas" {
  ...
  name  = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.all_roles[count.index]}"
...
resource "aws_iam_role_policy_attachment" "ppv_prod_ssm_policy" {
  count      = "${length(var.all_roles)}"
  role       = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.all_roles[count.index]}"
...
resource "aws_iam_role_policy_attachment" "route53_manage_private_hz" {
  ...
  role       = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.route53[count.index]}"
...
resource "aws_iam_role_policy_attachment" "s3_common_ro" {
  ...
  role       = "ppv_svcrole_${dev-tst-dem-int}_tenant${var.tenant_number}${var.s3_common[count.index]}"
