...
ALERT_AFTER_N_DAYS = os.environ.get("ALERT_AFTER_N_DAYS", 100)
# How ofter we have set the cron to run the Lambda?
SEND_EVERY_N_DAYS = os.environ.get("SEND_EVERY_N_DAYS", 3)
# Who send the email?
SES_SENDER_EMAIL_ADDRESS = os.environ.get("SES_SENDER_EMAIL_ADDRESS",
                                          'hector_bastos@mckinsey.com')
# Where did we setup SES?
SES_REGION_NAME = os.environ.get("SES_REGION_NAME", 'us-east-1')
...
"""
    ...
    try:
        ses_response = ses_client.send_email(
            ...
            Message={
                ...
                'Subject': {
                    ...
                    'UTF-8',
                    'Data':
                    f'Remember to rotate your AWS Keys on account {account_id}!'
                }
            },
        ...
        logging.info(
            f'Notification email sent successfully to {email}! Message ID: {ses_response["MessageId"]}'
        )
...
def main(event, context):
    ...
    while is_truncated:
        ...
        response = iam_client.list_users(
            **
            {k: v
             for k, v in (dict(Marker=marker)).items() if v is not None})
    ...
    for user in interesting_keys_grouped_by_user.values():
        ...
        # check
        send_notification(user[0]['UserName'], user,
